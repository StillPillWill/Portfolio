...
function NormalizedModel({ url, onReady }: { url: string; onReady: () => void }) {
  ...
  return (
    <group
      ...
      position={[normalized.position[0], normalized.position[1], normalized.position[2]]}
    >
...
function FitCamera({ rootRef }: { rootRef: RefObject<THREE.Group | null> }) {
  const controls = useThree((s) => s.controls as { update?: () => void; target?: THREE.Vector3 } | undefined);
  ...
  useFrame(({ camera }) => {
    ...
    // Ensure world matrix is computed before calculating bounding box
    ...
    let dist = (sphere.radius / Math.sin(fovRad / 2)) * 1.15;

    // Safety guard: normalized model should fit within dist 3-10
    if (!Number.isFinite(dist) || dist < 2 || dist > 15) {
      dist = 5.2;
    }

    camera.position.set(dist * 0.65, dist * 0.42, dist * 0.92);
    camera.lookAt(0, 0, 0);
...
export default function ViewerScene({ url, spinRef, onReady, onError }: ViewerSceneProps) {
  ...
  return (
    <GLTFBoundary onError={onError}>
      ...
      >
        {/* Studio rig: balanced ambient fill so shadow faces stay legible,
            strong directional key and chapter-tinted rim to model CAD form. */}
        ...
        />
        <directionalLight position={[5, 3, -4]} intensity={0.9} color="#a8ccff" />
        <directionalLight position={[2, -3, 4]} intensity={0.25} />
